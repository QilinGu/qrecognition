...
cv::Mat Util::convertToMat(const QImage &image)
{
    QImage rgb = image.convertToFormat(QImage::Format_RGB888);
    return cv::Mat(rgb.height(), rgb.width(), CV_8UC3, (void *)rgb.scanLine(0), rgb.bytesPerLine());
}

cv::Mat Util::convertToMat(const QVideoFrame &frame)
{
    return convertToMat(convertToImage(frame));
}

QImage Util::convertToImage(const cv::Mat &mat)
{
...
QImage Util::convertToImage(const QVideoFrame &frame)
{
